			...
			<section id="main" class="wrapper">
				<div class="container">
					...
					<!-- Text -->
						<section>
						<center>
						<form action="UpdateRDF" method="post">
<table cellpadding="15" cellspacing="15">
<%
JenaTest.Search3(request.getParameter("skey"));
...
for(String s:spv)
{
	...
	if(Integer.parseInt(sv[1])==1)
	{
		...
		if(val.length()<=15)
		{
		frm.add(""+sv[0]+"~"+"<input type='text' name='"+sv[0]+"' value='"+val+"'/>");
		}
		else
		{
			frm.add(""+sv[0]+"~"+"<textarea rows='5' cols='25' name='"+sv[0]+"'>"+val+"</textarea>");	
		}
	}
	else
	{
		...
		for(String ss:sp)
		{
			outp+="<option value='"+ss+"'>"+ss+"</option>";
		}
		frm.add(""+sv[0]+"~"+"<select name='"+sv[0]+"'>"+outp+"</select>");
	}
}
for(String s:frm)
{
